    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg z-10 overflow-hidden">
        ...
        <form id="add-form" onsubmit="handleAddUser(event)" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Nama Sekolah <span class="text-red-500">*</span></label>
                    <input type="text" name="institution_name" required placeholder="SMAN 1 Jakarta" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition bg-slate-50">
                </div>
            </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg z-10 overflow-hidden">
        ...
        <form id="edit-form" onsubmit="handleEditUser(event)" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Nama Sekolah <span class="text-red-500">*</span></label>
                    <input type="text" name="institution_name" id="edit-institution" required placeholder="SMAN 1 Jakarta" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#059669] transition bg-white">
                </div>
            </div>
    ...
    db.collection('superadmin').doc(SUPERADMIN_ID).collection('sekolah')
      .onSnapshot(snapshot => {
              ...
              if(hasSub && endMillis > 0) {
                  ...
                  if(diffDays > 0) {
                      ...
                      if (!hasAlertedExpired.has(doc.id)) {
                          ...
                          newlyExpired.push(user.institutionName || 'Sekolah');
                      }
                  }
              }
          ...
          if (newlyExpired.length > 0) {
              const text = newlyExpired.length > 1 ? `Langganan untuk ${newlyExpired.length} sekolah telah habis!` : `Langganan untuk ${newlyExpired[0]} telah habis!`;
...
function updateEditBadge() {
    ...
    const badge = document.getElementById('edit-status-badge');
    if (diffDays > 0) {
        badge.innerHTML = `${newEndDate.toLocaleDateString('id-ID', {day:'numeric',month:'short',year:'numeric'})} - Sisa ${diffDays} hari`;
        badge.className = 'text-[10px] bg-emerald-200 text-emerald-800 px-2.5 py-1 rounded-full font-bold';
    } else {
...
document.getElementById('add-duration-days').addEventListener('change', updateEditBadge);
document.getElementById('edit-duration-days').addEventListener('change', updateEditBadge);
