...
export default function UniversitasPage() {
  ...
  const bgGradient = "bg-gradient-to-br from-indigo-600 to-indigo-800";
    ...
    <div className="p-6 space-y-6 bg-[#f4f7fe] min-h-screen font-sans">
      ...
      <div className={`${bgGradient} rounded-3xl p-8 text-white shadow-lg border border-white/20 relative overflow-hidden`}>
        <div className="absolute top-0 right-0 p-8 opacity-10">
          <svg className="w-40 h-40 transform translate-x-4 -translate-y-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z" /></svg>
        </div>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => openEditModal(item)} title="Edit" className="p-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
            ...
            <div className="flex gap-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
                <button
                  ...
                  className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${currentPage === num ? 'bg-indigo-600 text-white shadow-md' : 'bg-white border border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                >{num}</button>
              ))}
            </div>
            <button
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white">
            ...
            <form id="editForm" onSubmit={handleEditSubmit} className="p-8 space-y-5">
              <div className="space-y-1.5">
                ...
                <input name="name" required className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3 text-sm focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Nama Institusi</label>
                <input name="institution_name" required className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3 text-sm focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium" />
              </div>
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" className="flex-1 bg-indigo-600 text-white py-3 rounded-2xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all">Simpan</button>
              </div>
            </form>
          </div>
        </div>
