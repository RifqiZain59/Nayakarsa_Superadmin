...
export default function PerusahaanPage() {
  ...
  const title = "Daftar Perusahaan";
  const bgGradient = "bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-900";
  const badgeClass = "bg-emerald-50 text-emerald-600 border border-emerald-100";
    ...
    try {
      ...
      if (isEditMode && editItemId) {
        ...
        await addDoc(collection(db, "superadmin", superadminId, "logs"), {
          action: "Edit Data",
          message: `Data penanggung jawab ${name} telah diperbarui di Perusahaan`,
        ...
        await addDoc(collection(db, "superadmin", superadminId, "logs"), {
          action: "Tambah Data",
          message: `Data penanggung jawab ${name} berhasil didaftarkan di Perusahaan`,
          timestamp: serverTimestamp()
        });
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    if (result.isConfirmed) {
      ...
      await addDoc(collection(db, "superadmin", superadminId, "logs"), {
        action: "Hapus Data",
        message: `Data ${name} telah dihapus dari Perusahaan`,
        timestamp: serverTimestamp()
      });
  ...
  return (
    <div className="p-6 space-y-6 bg-[#f4f7fe] min-h-screen font-sans">
      ...
      <div className={`${bgGradient} rounded-[2rem] p-8 sm:p-10 text-white shadow-xl relative overflow-hidden border border-white/10`}>
        ...
        <div className="absolute bottom-0 left-0 w-72 h-72 bg-teal-400/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4"></div>
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6 z-10">
          <div className="flex items-center gap-5 sm:gap-6">
            ...
            <div>
              ...
              <p className="text-emerald-50 mt-1 text-sm sm:text-base font-medium max-w-xl leading-relaxed text-balance">
                Kelola akses, konfigurasi langganan, dan pantau aktivitas data institusi perusahaan yang terdaftar.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="group w-full md:w-auto bg-white text-emerald-700 px-6 py-3.5 rounded-2xl font-bold transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_10px_30px_rgba(52,211,153,0.3)] flex items-center justify-center gap-3 border border-emerald-100 shadow-sm"
          >
            <div className="bg-emerald-100/80 p-1.5 rounded-xl group-hover:rotate-90 group-hover:bg-emerald-200 transition-all duration-300">
              <svg className="w-5 h-5 text-emerald-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
            </div>
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <select
                ...
                className="border border-gray-300 rounded-lg pl-3 pr-8 py-1.5 text-sm focus:ring-2 focus:ring-emerald-500 outline-none cursor-pointer appearance-none bg-white"
              >
          ...
          <div className="relative w-full md:w-72">
            <input
              ...
              className="w-full bg-gray-50 border border-gray-300 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:bg-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
            />
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => openEditModal(item)} title="Edit" className="p-2 bg-gray-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
            ...
            <div className="flex gap-1 hidden sm:flex">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-sm transition-all ${currentPage === num ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'}`}
                >{num}</button>
              ))}
            </div>
            <button
            ...
            <div className="w-full md:w-7/12 flex flex-col max-h-[85vh]">
              ...
              <form id="dataForm" onSubmit={handleAddSubmit} className="flex flex-col flex-1 overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div className="space-y-1.5 md:col-span-2">
                        ...
                        <div className="relative group">
                          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-emerald-500">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                          </div>
                          <input name="name" required placeholder="Contoh: Budi Santoso" className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all shadow-sm" />
                        </div>
                      </div>
                    ...
                    <div className="space-y-5">
                      <div className="space-y-1.5">
                        ...
                        <div className="relative group">
                          ...
                          <input name="institution_name" required placeholder={`Contoh: PT. Nayakarsa`} className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all shadow-sm" />
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative group">
                          ...
                          <select name="subscription_duration" required className="w-full pl-11 pr-10 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all shadow-sm appearance-none cursor-pointer font-medium text-gray-700">
                ...
                <div className="flex justify-end gap-3 px-8 py-5 bg-white border-t border-gray-100 shrink-0">
                  ...
                  <button type="submit" className="px-8 py-2.5 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all shadow-md hover:shadow-lg flex items-center gap-2">
